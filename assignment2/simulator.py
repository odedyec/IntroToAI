...
class HurricaneSimulator:
    ...
    def add_people_to_vehicle(self):
        self.current_agent.add_people_to_vehicle(self.graph[self.current_agent.get_state()][self.current_agent.get_state()].num_of_people)
        self.graph[self.current_agent.get_state()][self.current_agent.get_state()].num_of_people = 0

    def time_cost(self, new_state):
        return self.graph[self.current_agent.get_state()][new_state].weight * (1 + self.K * self.current_agent.people_in_vehicle)

    def traverse(self, new_state, should_pick_and_drop=True):
        cost = self.time_cost(new_state)
        if cost < 0:
            print("No road from ", self.current_agent.get_state(), "to", new_state, ". \nAction ignored")
            return
        self.num_of_actions += 1
        self.last_action_cost = cost
        self.time += self.last_action_cost
        self.current_agent.set_state(new_state)
        ...
        if should_pick_and_drop:
            if self.graph[new_state][new_state].is_safe:
                self.current_agent.reached_shelter()
            else:
    ...
    def print_all(self):
        ...
        print("Current state ", [ag.get_state() for ag in self.all_agents])
        print('Number of people in vehicle: ', [ag.people_in_vehicle for ag in self.all_agents])
        print("Num of actions is :", self.num_of_actions)
        print("Saved  people: ", [ag.people_saved for ag in self.all_agents])
    ...
    def apply_action(self, action):
        if action == -1 or self.current_agent.get_state() == action:
            self.no_op()
            ...
            self.traverse(action, True)
        self.agent_index = (self.agent_index + 1) % len(self.all_agents)
        self.current_agent = self.all_agents[self.agent_index]
